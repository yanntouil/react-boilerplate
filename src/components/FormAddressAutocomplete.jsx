 ...
 *          address: string,
 *          coordinates: number[],
 *          error: 'api-error'|'geolocation-disabled'|'geolocation-unavailable'|'geolocation-permission-denied'|'geolocation-api-error',
 *      }
...
const FormAddressAutocomplete = ({ 
    ...
    setValue,
    placeholder = '', 
    name = 'form-address-autocomplete', 
    geolocation = false,
    clear = false,
    isValid, 
    validate,
     ...
     * Field
     */
    const [ fieldValue, setFieldValue ] = useState(value.address)
    const fieldRef = useRef(null)
    const fieldChange = async ({ target }) => {
        setFieldValue(target.value)
        setValue({ coordinates: [], address: '', error: '' })
        setGeolocated('off')
        setDropdown(true)
        try {
            ...
            if (option) setValue({ coordinates: option.coordinates, address: option.value, error: '' })
        } catch (error) {
            setValue({ coordinates: [], address: '', error: 'api-error' })
        }
    }
    ...
    const selectOption = useCallback((option) => {
        ...
        setValue({ coordinates: option.coordinates, address: option.value, error: '' })
        fieldRef?.current?.focus()
        setHoveredOption(-1)
        setDropdown(false)
    }, [ setValue ])
    ...
    useEffect(() => {// Replace hovered option
        if (!dropdown) return
     ...
     * Geolocation
     */
    const [ geolocated, setGeolocated ] = useState('off') 
    const geolocationError = useCallback((error) => {
        setGeolocated(error)
        setValue({ ...value, error: 'geolocation-' + error })
    }, [setValue, value])
    const onClickGeolocation = useCallback(() => {
        setGeolocated('pending')
        ...
        navigator.geolocation.getCurrentPosition(async ({ coords }) => {
            try {
                ...
                setValue({ coordinates: location.coordinates, address: location.address, error: '' })
            } catch (error) {
                geolocationError('api-error')
            }
        }, (error) => geolocationError(error.code === 1 ? 'permission-denied' : 'unavailable'))
    }, [ geolocationError, setValue ])
    ...
    return (
        ...
        >
            ...
            <input 
                ...
                name={name}
                className={classNames([
                    'grow h-full pl-16 leading-4 bg-white shadow placeholder:text-neutral-500 text-lg placeholder:text-neutral-500',
                    geolocation ? 'pr-16' : 'pr-4',
                    {
